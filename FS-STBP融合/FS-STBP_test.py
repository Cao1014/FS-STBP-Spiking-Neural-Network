...
def fs_mem_update(z, d, fx):  # h0_spike,  d[step]
    fx += z * d
    return fx                  # h1
...
class CNN_to_SNN(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        h0_spikes = fx1 = torch.zeros(batch_size, cfg_fc[0], device=device)  # 121
        h1_spikes = h1 = fx2 = torch.zeros(batch_size, cfg_fc[1], device=device)  # 64
        h2_spikes = h2 = fx3 = torch.zeros(batch_size, cfg_fc[2], device=device)  # 124
        h3_spikes = h3 = fx4 = torch.zeros(batch_size, cfg_fc[3], device=device)  # 64
        h4_spikes = h4 = fx5 = torch.zeros(batch_size, cfg_fc[4], device=device)  # 124
        h5_spikes = h5 = fx6 = torch.zeros(batch_size, cfg_fc[5], device=device)  # 10
        ...
        for step in range(K[0]):
            ...
            # 更新c1_mem
            h0_spikes += h0_spike
            fx1 = fs_mem_update(h0_spike, d1[step], fx1)
        ...
        for step in range(K[1]):
            ...
            # 更新c2_mem
            fx2 = fs_mem_update(h1_spike, d2[step], fx2)
        ...
        for step in range(K[2]):
            ...
            # 更新h1_mem
            fx3 = fs_mem_update(h2_spike, d3[step], fx3)
        ...
        for step in range(K[3]):
            ...
            h3_spikes += h3_spike
            # 更新h2_mem
            fx4 = fs_mem_update(h3_spike, d4[step], fx4)
            # h22 = h2.cpu().numpy()
        ...
        for step in range(K[4]):
            ...
            h4_spikes += h4_spike
            # 更新h3_mem
            fx5 = fs_mem_update(h4_spike, d5[step], fx5)
