...
#  Iters: 32
# Epoch [33/80], Step [100/600], Loss: 0.36131,Time elasped：0.87390
# Epoch [33/80], Step [200/600], Loss: 0.38012,Time elasped：1.68466
# Epoch [33/80], Step [300/600], Loss: 0.37764,Time elasped：2.45538
# Epoch [33/80], Step [400/600], Loss: 0.40209,Time elasped：3.24223
# Epoch [33/80], Step [500/600], Loss: 0.38526,Time elasped：4.00692
# Epoch [33/80], Step [600/600], Loss: 0.40746,Time elasped：4.88576
# 50 sh1:0.04458, sh2:0.00000
# 100 sh1:0.04132, sh2:0.00000
#
#  Best test Accuracy of the snn on the 10000 test images: 97.910
# Average spikes:sc1_ave:0.04166, sc2_ave:0.00000
...
K = [4, 4, 3, 3, 2]
alpha = [1, 3, 3, 2, 1]  # 根据原先网路中relu层输出最大值来设置
...
# fs-neuron 更新膜电位
def fs_mem_update(ops, x, mem):  # self.fc1, h0_spike, h1(zeros)
    mem = mem + ops(x)
    return mem                   # h1
...
class CNN_to_SNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(cfg_fc[0], cfg_fc[1])          # 784 400
        self.fc2 = nn.Linear(cfg_fc[1], cfg_fc[2])          # 400 10
    ...
    # def forward(self, input: torch.Tensor, c1, c2, h1, h2, h3):
    def forward(self, input: torch.Tensor):
        ...
        h0_spikes = torch.zeros(batch_size, cfg_fc[0], device=device)  # 784
        h1_spikes = h1 = torch.zeros(batch_size, cfg_fc[1], device=device)  # 400
        h2_spikes = h2 = torch.zeros(batch_size, cfg_fc[2], device=device)  # 10
        ...
        for step in range(K[0]):
            ...
            # 更新c1_mem
            h0_spikes += h0_spike
            h1 = fs_mem_update(self.fc1, h0_spike, h1)
        ...
        for step in range(K[1]):
            ...
            # 更新c2_mem
            h2 = fs_mem_update(self.fc2, h1_spike, h2)
        ...
        #     # 更新h1_mem
        #     h3 = fs_mem_update(self.fc3, h2_spike, h3)
        ...
        #     h3_spikes += h3_spike
        #     # 更新h2_mem
        #     h4 = fs_mem_update(self.fc4, h3_spike, h4)
        #     # h22 = h2.cpu().numpy()
        ...
        #     h4_spikes += h4_spike
        #     # 更新h3_mem
        #     h5 = fs_mem_update(self.fc5, h4_spike, h5)
        #     #self.fc5.weight.data = self.fc5.weight.data * 0.5


        #统计该轮平均脉冲数
        sh1 = torch.sum(h0_spikes) / (cfg_fc[0] * batch_size)  # 784x100
        sh2 = torch.sum(h1_spikes) / (cfg_fc[1] * batch_size)  # 400x100
        # sh3 = torch.sum(h3_spikes) / (cfg_fc[2] * batch_size)  # 64x100
        # sh4 = torch.sum(h4_spikes) / (cfg_fc[3] * batch_size)  # 124x100
        # sh5 = torch.sum(h5_spikes) / (cfg_fc[4] * batch_size)  # 10x100
        # s_ave = (torch.sum(h1_spikes) + torch.sum(h2_spikes) + torch.sum(h3_spikes) + torch.sum(h4_spikes) + torch.sum(
        #     h5_spikes)) / (cfg_fc[0] * batch_size + cfg_fc[1] * batch_size +
        #                    cfg_fc[2] * batch_size + cfg_fc[3] * batch_size + cfg_fc[4] * batch_size)
        s_ave = (torch.sum(h0_spikes) + torch.sum(h1_spikes)) / (cfg_fc[0] * batch_size + cfg_fc[1] * batch_size )

        # 最后一层不发放脉冲，直接使用膜电压分类
        outputs = h2
        return outputs, sh1, sh2,  s_ave
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        outputs, sh1, sh2,  s_average = snn(images)  # 这里改成求平均脉冲数
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            optimizer.zero_grad()
            outputs, sh1, sh2,  s_average = snn(inputs)  # 这里改成求平均脉冲数
            ...
            if (batch_idx + 1) % 50 == 0:
                print(batch_idx + 1, "sh1:%.5f, sh2:%.5f" % (sh1, sh2 ))
    ...
    if acc > best_acc:  # 最佳准确率
        ...
        print("Average spikes:sc1_ave:%.5f, sc2_ave:%.5f"
         % (sh1_sum / (batch_idx + 1), sh2_sum / (batch_idx + 1)), '\n')
        best_acc = acc
